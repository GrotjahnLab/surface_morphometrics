...
from pycurv import vector_voting as vv
import numpy as np
from scipy import ndimage
from graph_tool import load_graph
import time

fold = r"/gpfs/group/grotjahn/bbarad/filled/" # output will be also written there
base_filename = "mito_test"
seg_file = "Apr20_TS02_4bin_SIRTl20_rotx_mito.mrc" # MRC in this example
filled_label = 1
...
cores = 16  # number of cores to run VV in parallel (default 6)
print("Loading Segmentation at {}".format(time.time()))
seg = io.load_tomo(fold + seg_file)
data_type = seg.dtype
print("Making Binary at {}".format(time.time()))
filled_binary_seg = (seg == filled_label).astype(data_type)
print(np.sum(filled_binary_seg > 0))
surf = run_gen_surface(filled_binary_seg, fold + base_filename, lbl=1, isosurface=True, sg=1, thr=THRESH_SIGMA1)
...
method_tg_surf_dict = vv.normals_directions_and_curvature_estimation(
    tg, radius_hit, methods=methods, area2=area2, cores=cores,
    poly_surf=surf_clean) 
