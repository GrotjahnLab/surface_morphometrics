...
from morphometrics_stats import Experiment, Tomogram, weighted_median, weighted_histogram_peak, statistics, histogram, twod_histogram, barchart, double_barchart, ks_statistics
...
startdir = os.getcwd()
output_dir = os.getcwd()+"/output_split/"
...
for file in untreated, treated:
    ...
    for key,val in file.tomograms.items():
        ...
        if key[1] == "F":
            ...
            distance_set.extend(val["OMM"]["IMM_dist"])
            area_set.extend(val["OMM"]["area"])
            histogram([val["OMM"]["IMM_dist"]], [val["OMM"]["area"]], [key], f"IMM-OMM Dist ({key})", "Distance (nm)", filename="IMM-OMM_dist_"+key+".svg", bins=100, range=[5,25], legend=False)
    distance_peak_sets.append(peak_set)
    distances.append(distance_set)
...
for feature in ["crista", "junction", "IBM", "IMM"]:
    ...
    ks_statistics(close, all_areas, ns, conditions, morphs, basename=f"Intra{feature} spacing", filename=f"intra{feature} spacing_ks.csv")
    statistics(far_peaks, basename_far, conditions, morphs, test_type="Peaks", ylabel="Distance (nm)", filename=filename_far)
    ks_statistics(far, all_areas, ns, conditions, morphs, basename=f"Inter{feature} spacing", filename=f"inter{feature} spacing_ks.csv")
# for run in ['elongated', 'fragmented', 'all']:
#     curvature_list = []
#     log_curvature_list = []
#     area_list = []
